...
class Product(models.Model):
    type = models.ForeignKey(Category, on_delete=models.CASCADE)  # this argument "on_delete=models.CASCADE" is important when 
    ...
    slug = models.SlugField(unique=True)  # this argument "unique=True" is important when using slungfiel
    color = models.CharField(max_length=50, default='a') # fieldname color was not included when we make last migration, so far 
    ...
    Register = models.BooleanField(default='True') # fieldname register was not included when we make last migration, so far 
    ...
    promo_price = models.FloatField(blank=True, null=True)  # this arguement "blank=True, null=True" means not required, either information is provided or not it will be saved.
    car_photo = models.ImageField(upload_to='car_photo')
    uploaded_at = models.DateTimeField(auto_now_add=True)  # the argument "auto_now_add=True" display the date and time the info was
    # first uploaded,
    uploaded_at = models.DateTimeField(auto_now=True) #  while "auto_now=True" display the time and date when the info was edited or updated.
